...
setwd("/Users/apple/Downloads/videos 2 y 3 2")
...
qtm(lnd, "Crimep1000") # plot the basic map
qtm(shp = lnd, fill = "Crimep1000", fill.palette = "-Reds", fill.title = "Crimen por 1000 habitantes") 

qtm(shp = lnd, fill = "Crimep1000", fill.palette = "Reds", fill.title = "Crimen por 1000 habitantes", fill.style="quantile") 
...
map <- ggplot(lnd_f, aes(long, lat, group = group, fill = Crimep1000)) +
  geom_polygon() + coord_equal() +
  labs(x = "Easting (m)", y = "Northing (m)",
       fill = "Crimen por 1000 hab") +
  ggtitle("Crimen en Londres")
